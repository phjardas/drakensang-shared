...
public final class DaoHelper {
	public static void load(Person person, ResultSet result)
		...
		person.setRace(Race.valueOf(result.getString("Race")));
		person.setCulture(Culture.valueOf(result.getString("Culture")));
		person.setProfession(Profession.forProfession(result
				.getString("Profession")));
		person.setMagician(result.getInt("IsMagicUser") == 1);
		person
				.setCasterType(CasterType.valueOf(result
						.getString("CasterType")));
		person
				.setCasterRace(CasterRace.valueOf(result
	...
	public static void load(Regenerating reg, ResultSet result)
		...
		reg.setRegenerationFrequency(result.getInt("Reg_" + reg.getName()
				+ "_freq"));
		reg.setRegenerationFrequencyCombat(result.getInt("Reg_" + reg.getName()
				+ "_freq_combat"));
	...
	public static <I extends Enum<I>> Set<I> parseList(String string,
			Class<I> type) {
	...
	public static void store(Regenerating reg, UpdateStatementBuilder builder) {
		...
		builder.append("'Reg_" + reg.getName() + "' = ?", reg
				.getRegenerationAmount());
		builder.append("'Reg_" + reg.getName() + "_freq' = ?", reg
				.getRegenerationFrequency());
		builder.append("'Reg_" + reg.getName() + "_freq_combat' = ?", reg
	...
	public static void store(Person person, UpdateStatementBuilder builder) {
		...
		builder.append("'Race' = ?", person.getRace().name());
		builder.append("'Culture' = ?", person.getCulture().name());
		builder.append("'Profession' = ?", person.getProfession()
				.getProfession());
