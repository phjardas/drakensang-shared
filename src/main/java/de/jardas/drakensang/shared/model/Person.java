...
public abstract class Person extends Persistable implements PropertyChangeProducer {
    ...
    private final DerivedInteger attackeBasis =
        new DerivedInteger(this) {
            @Override
            protected void updateDerivedValue(Person character) {
                setValue((int) Math.round(
                        (double) (getAttribute().get("MU") + getAttribute().get("GE") +
                        getAttribute().get("KK")) / 5));
            }
        };

    private final DerivedInteger paradeBasis =
        new DerivedInteger(this) {
            @Override
            protected void updateDerivedValue(Person character) {
                setValue((int) Math.round(
                        (double) (getAttribute().get("IN") + getAttribute().get("GE") +
                        getAttribute().get("KK")) / 5));
            }
            ;
        };

    private final DerivedInteger fernkampfBasis =
        new DerivedInteger(this) {
            @Override
            protected void updateDerivedValue(Person character) {
                setValue((int) Math.round(
                        (double) (getAttribute().get("IN") + getAttribute().get("FF") +
                        getAttribute().get("KK")) / 5));
            }
            ;
        };

    private final DerivedInteger magieresistenz =
        new DerivedInteger(this) {
            @Override
            protected void updateDerivedValue(Person character) {
                setValue((int) Math.round(
                        (double) (getAttribute().get("MU") + getAttribute().get("KL") +
                        getAttribute().get("KO")) / 5) + getRace().getMagieresistenzModifikator() +
    ...
    protected Person(boolean withLEBonus, boolean withAEBonus) {
        ...
        attribute.addPropertyChangeListener(new ForwardingPropertyChangeListener("attributes."));
        sonderfertigkeiten.addPropertyChangeListener(new ForwardingPropertyChangeListener(
                "sonderfertigkeiten."));
        zauberfertigkeiten.addPropertyChangeListener(new ForwardingPropertyChangeListener(
                "zauberfertigkeiten."));
        talente.addPropertyChangeListener(new ForwardingPropertyChangeListener("talente."));
        advantages.addPropertyChangeListener(new ForwardingPropertyChangeListener("advantages."));
        lebensenergie.addPropertyChangeListener(new ForwardingPropertyChangeListener(
                "lebensenergie."));
        astralenergie.addPropertyChangeListener(new ForwardingPropertyChangeListener(
                "astralenergie."));
        ausdauer.addPropertyChangeListener(new ForwardingPropertyChangeListener("ausdauer."));
        attackeBasis.addPropertyChangeListener(new ForwardingPropertyChangeListener("attackeBasis."));
        paradeBasis.addPropertyChangeListener(new ForwardingPropertyChangeListener("paradeBasis."));
        fernkampfBasis.addPropertyChangeListener(new ForwardingPropertyChangeListener(
                "fernkampfBasis."));
        magieresistenz.addPropertyChangeListener(new ForwardingPropertyChangeListener(
                "magieresistenz."));
    }
    ...
    public void setProfession(Profession profession) {
        firePropertyChange("profession", profession, this.profession = profession);
    }
    ...
    public void setLookAtText(String lookAtText) {
        firePropertyChange("lookAtText", this.lookAtText, this.lookAtText = lookAtText);
    }
    ...
    public void setMagician(boolean magician) {
        if (magician != this.magician) {
            firePropertyChange("magician", this.magician, this.magician = magician);
        }
    }
    ...
    public void setCasterType(CasterType casterType) {
        firePropertyChange("casterType", this.casterType, this.casterType = casterType);
    }
    ...
    public void setCasterRace(CasterRace casterRace) {
        firePropertyChange("casterRace", this.casterRace, this.casterRace = casterRace);
    }
    ...
    public void setSneakSpeed(double sneakSpeed) {
        firePropertyChange("sneakSpeed", this.sneakSpeed, this.sneakSpeed = sneakSpeed);
    }
    ...
    public void setWalkSpeed(double walkSpeed) {
        firePropertyChange("walkSpeed", this.walkSpeed, this.walkSpeed = walkSpeed);
    }
    ...
    public void setCurrentSpeed(double currentSpeed) {
        firePropertyChange("currentSpeed", this.currentSpeed, this.currentSpeed = currentSpeed);
    }
    ...
    protected void firePropertyChange(String property, Object oldValue, Object newValue) {
        ...
        if ((property != null) && LOG.isDebugEnabled()) {
            LOG.debug("Property {}: {} --> {}", new Object[] { property, oldValue, newValue });
        }
    ...
    public void addPropertyChangeListener(PropertyChangeListener listener, String... properties) {
        if ((properties != null) && (properties.length > 0)) {
            listeners.addPropertyChangeListener(new ConfinedPropertyChangeListener(listener,
                    properties));
        } else {
    ...
    private static class ConfinedPropertyChangeListener implements PropertyChangeListener {
        private final PropertyChangeListener listener;
        private final String[] properties;

        public ConfinedPropertyChangeListener(PropertyChangeListener listener, String[] properties) {
        ...
        private boolean isInterested(PropertyChangeEvent evt) {
            ...
            for (String property : properties) {
                ...
                if (property.endsWith("*") &&
                        propertyName.startsWith(property.substring(0, property.indexOf("*")))) {
                    return true;
                }
            }
    ...
    private class ForwardingPropertyChangeListener implements PropertyChangeListener {
        ...
        public void propertyChange(PropertyChangeEvent evt) {
            firePropertyChange(prefix + evt.getPropertyName(), evt.getOldValue(), evt.getNewValue());
        }
    }
}
