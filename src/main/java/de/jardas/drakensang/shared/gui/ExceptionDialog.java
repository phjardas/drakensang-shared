...
public class ExceptionDialog extends JDialog {
    ...
    public ExceptionDialog(final Frame parent, Throwable throwable) {
        ...
        add(new JLabel("Unfortunately an application failure has occurred."), BorderLayout.NORTH);
        add(new JScrollPane(new JTextArea(trace.toString())), BorderLayout.CENTER);

        final JPanel toolbar = new JPanel();
        toolbar.add(new JButton(new AbstractAction("Send anonymous error report") {
                public void actionPerformed(ActionEvent e) {
                    try {
                        final URL url = new URL(applicationInfo.getBugreportUrl());
                        final String query =
                                "application=" +
                                URLEncoder.encode(applicationInfo.getApplicationName(), "utf-8") +
                                "&version=" +
                                URLEncoder.encode(applicationInfo.getVersion(), "utf-8") +
                                "&build=" + URLEncoder.encode(applicationInfo.getBuild(), "utf-8") +
                                "&lastFeature=" + FeatureHistory.getLatestFeatureId() +
                                "&stacktrace=" + URLEncoder.encode(trace.toString(), "utf-8") +
                                "&log=" + URLEncoder.encode(LogCollector.getLog(), "utf-8") +
                                "&properties=" + URLEncoder.encode(getSystemProperties(), "utf-8");
                        ...
                        JOptionPane.showMessageDialog(parent,
                                "Your error report has been submitted.", "Thank you!",
                        ...
                        JOptionPane.showMessageDialog(parent,
                                ...
                                "Thank you.", "Sorry...", JOptionPane.ERROR_MESSAGE);
                    }
                }
            }));
